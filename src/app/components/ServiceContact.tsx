...
const HeroSection = () => {
  ...
  const resetAnimations = () => {
    ...
    if (subtextRef.current) {
      subtextRef.current.classList.remove('animate-subtext');
    }
    if (buttonsRef.current) {
      buttonsRef.current.classList.remove('animate-buttons');
    }
    if (mockup1Ref.current) {
      mockup1Ref.current.classList.remove('animate-mockup-1');
    }
    if (mockup2Ref.current) {
      mockup2Ref.current.classList.remove('animate-mockup-2');
    }
  ...
  const triggerAnimations = () => {
    // Small delay to ensure reset has taken effect
    setTimeout(() => {
      if (cardRef.current) {
      ...
      if (subtextRef.current) {
        subtextRef.current.classList.add('animate-subtext');
      }
      if (buttonsRef.current) {
        buttonsRef.current.classList.add('animate-buttons');
      }
      if (mockup1Ref.current) {
        mockup1Ref.current.classList.add('animate-mockup-1');
      }
      if (mockup2Ref.current) {
        mockup2Ref.current.classList.add('animate-mockup-2');
      }
    }, 50);
  };
  ...
  useEffect(() => {
    // Intersection Observer to detect when the section enters/exits viewport
    const observer = new IntersectionObserver(
      ...
      { 
        ...
        rootMargin: '-50px 0px -50px 0px' // Add some margin to make animation more responsive
      }
    );
    ...
    if (isVisible && mockup1Ref.current && mockup2Ref.current) {
      ...
      const floatTimer = setTimeout(() => {
        const startFloatingAnimation = () => {
          ...
          if (mockup1 && mockup2 && isVisible) {
            let startTime: number | null = null;
            const animate = (timestamp: number) => {
              if (!startTime) startTime = timestamp;
              const elapsed = timestamp - startTime;
              const translateY1 = Math.sin(elapsed / 3000) * 6;
  ...
  return (
    <>
      ...
      <style jsx>{`
        ...
        /* Enhanced Button Container - Single button layout */
        .button-container {
          display: flex;
          opacity: 0;
          transform: translateX(-50px);
        ...
        /* Enhanced Primary Button - More professional and transparent */
        .primary-button {
          padding: 1rem 2.5rem;
          font-size: 1.1rem;
          font-weight: 600;
          ...
          border: 2px solid rgba(13, 152, 186, 0.6);
          color: #FFFFFF;
          box-shadow: 
            0 8px 25px rgba(13, 152, 186, 0.15),
        ...
        .primary-button:hover {
          ...
          border-color: rgba(13, 152, 186, 0.8);
          box-shadow: 
            0 12px 30px rgba(13, 152, 186, 0.25),
        ...
        /* Mockups Container - Desktop/Tablet only */
        .mockups-container {
          position: relative;
          height: 450px;
          z-index: 1;
        ...
        /* Mockup Cards - Desktop/Tablet only */
        .mockup-card {
          position: absolute;
          opacity: 0;
