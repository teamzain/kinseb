...
// 3. Add a handleCardClick function before your return statement
const handleCardClick = (serviceSlug) => {
  router.push(`/services/${serviceSlug}`);
}
  // Default card content data with improved semantic structure
const defaultCards = [
{
  icon: "/images/icon.png", // Update with appropriate icon
  title: "Custom Web Development",
  description: "Tailored web development solutions covering both frontend and backend. We build high-performance, scalable websites and web apps designed around your unique business goals.",
  slug: "custom-web-development"
},
  {
    icon: "/images/web-design-icon.svg",
    title: "Web Design",
    description: "Professional web design that creates a stunning visual experience. We focus on UI/UX design that engages visitors and effectively communicates your brand message.",
    slug: "web-design"
  },
{
  icon: "/images/mobile-app-icon.svg",
  title: "Mobile App Design",
  description: "User-centric mobile app design focused on intuitive interfaces, seamless interactions, and visually compelling experiences for both iOS and Android platforms.",
...
,
  {
    ...
    description: "Complete website building services from concept to launch. We create responsive websites optimized for performance and search engines.",
    slug: "website-development"
  },
{
  icon: "/images/ecommerce-icon.svg",
  title: "eCommerce Development",
  description: "End-to-end eCommerce development services to build scalable, secure, and high-performing online stores tailored to your business needs.",
  slug: "ecommerce-development"
}
,
  {
    icon: "/images/custom-modules-icon.svg",
    title: "Custom Module Development",
    description: "Development of specialized custom modules and functionality for your existing websites or applications to extend capabilities and improve user experience.",
    slug: "custom-modules"
  }
];

  // Default section heading and description
  const defaultSectionHeading = "Why Kinseb Web Development?";
  const defaultSectionDescription = "Your One-Stop Web Partner for Growth, Design & Performance";

  // State for current cards, heading, and description to use
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch data from the homewhyus table
        const { data, error } = await supabase
          .from('homewhyus')
          .select('*')
          .single(); // Assuming there's only one row of configuration
        ...
        setError(err);
        // On error, we'll fall back to the default values
      } finally {
        setIsLoading(false);
      }
    };
  ...
  // Process the fetched data once available
  useEffect(() => {
    if (!isLoading && sectionData) {
      // Update section heading and description if available
      if (sectionData.section_heading) {
        setSectionHeading(sectionData.section_heading);
      }
      
      if (sectionData.section_description) {
        setSectionDescription(sectionData.section_description);
      }

      // Update cards data if available
      const dynamicCards = [...defaultCards];
      let hasCardData = false;

      for (let i = 1; i <= 6; i++) {
        const titleKey = `card${i}_title`;
        const descriptionKey = `card${i}_description`;

        if (sectionData[titleKey]) {
          dynamicCards[i - 1].title = sectionData[titleKey];
          hasCardData = true;
        }

        if (sectionData[descriptionKey]) {
          dynamicCards[i - 1].description = sectionData[descriptionKey];
          hasCardData = true;
        }
      }

      // Only update cards if we found data for at least one card
      if (hasCardData) {
  ...
  return (
    <>
      ...
      <Head>
        ...
        <meta property="og:url" content="https://kinsebwebdevelopment.com/services" />
        <meta property="og:image" content="https://kinsebwebdevelopment.com/images/kinsebwebdevelopment-services-og.jpg" />
        ...
        <meta name="twitter:image" content="https://kinsebwebdevelopment.com/images/kinsebwebdevelopment-services-twitter.jpg" />
        
        {/* Canonical URL to prevent duplicate content issues */}
        <link rel="canonical" href="https://kinsebwebdevelopment.com/services" />
      ...
      <Script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            "url": "https://kinsebwebdevelopment.com/services",
            "provider": {
              "@type": "Organization",
              ...
              "logo": {
                "@type": "ImageObject",
                "url": "https://kinsebwebdevelopment.com/images/logo.png"
              },
              "contactPoint": {
                "@type": "ContactPoint",
  ...
  {sectionHeading.split(' ').map((word, i) => (
    word.toLowerCase().includes('kinseb') 
      ? <span key={i} style={{ color: '#30B8DB' }}>{word} </span> 
      : <span key={i}>{word} </span>
          ...
          >
            ...
            <div className="services-cards">
              {isLoading ? (
                // Loading state - show placeholder cards with shimmer effect
                Array(3).fill(0).map((_, index) => (
                  <div 
                    key={`loading-${index}`}
                    className="service-card loading-card"
                    style={{
                  ...
                  >
                    <div className="shimmer-wrapper" style={{ width: '100%', height: '100%' }}>
  ...
  data-index={index}
  onClick={() => handleCardClick(card.slug)} // Add this onClick handler
  style={{
    backgroundColor: 'rgba(9, 40, 75, 0.75)',
    padding: '40px',
    borderRadius: '16px',
...
>
        ...
        <style jsx global>{`
          ...
          * {
          ...
          /* Loading animation for card placeholders */
          @keyframes shimmer {
            0% {
  ...
  transition: all 0.3s ease;
          ...
          /* Add these styles to your existing <style jsx global> section */
...
.service-card:hover {
  transform: translateY(-5px);
  box-shadow: 0 20px 40px rgba(0, 0, 0, 0.4), 0 0 25px rgba(48, 184, 219, 0.3);
  border: 1px solid rgba(48, 184, 219, 0.5);
}
...
/* Add to show clickable state more clearly */
.service-card:after {
  content: "";
  position: absolute;
  bottom: 20px;
