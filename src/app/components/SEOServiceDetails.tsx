...
import { SEOServiceData, getSEOServiceById } from './seoServiceData';
...
interface SEOServiceDetailsProps {
  serviceId: string;
}

const SEOServiceDetails: React.FC<SEOServiceDetailsProps> = ({ serviceId }) => {
  const router = useRouter();
  const [service, setService] = useState<SEOServiceData | null>(null);
  ...
  useEffect(() => {
    const loadService = async () => {
      try {
        ...
        const serviceData = await getSEOServiceById(numericId);
        if (serviceData) {
  ...
  if (loading) {
    return (
      <div className={`${poppins.variable} ${barlow.variable} ${lato.variable} loading-container`}>
        ...
        <p>Loading SEO service details...</p>
