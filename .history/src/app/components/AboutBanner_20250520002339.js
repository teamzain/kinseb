...
// Default values for the content (fallback if Supabase fetch fails)
const DEFAULT_CONTENT = {
  subtitle: "Seasoned Professionals, Proven Results",
  title: "Your Trusted <span style={{ color: COLORS.primary, textShadow: `0 0 10px ${COLORS.primaryLight}` }}>Project</span> Partners",
  ...
  statItem2: {
    title: "", // Empty as it's using counter
...
const Banner = () => {
    ...
    const fetchAboutHeroData = async () => {
      ...
      try {
        ...
        if (error) {
          ...
          // Fall back to default content (already set in initial state)
          setIsLoading(false);
          return;
        }

        if (data) {
          // Update content with data from Supabase
          setContent({
            ...
            statItem1: {
              title: data.statitem1_title || DEFAULT_CONTENT.statItem1.title,
              subtitle: data.statitem1_subtitle || DEFAULT_CONTENT.statItem1.subtitle
            },
            statItem2: {
              title: data.statitem2_title || DEFAULT_CONTENT.statItem2.title,
              subtitle: data.statitem2_subtitle || DEFAULT_CONTENT.statItem2.subtitle
            },
            statItem3: {
              title: data.statitem3_title || DEFAULT_CONTENT.statItem3.title,
              subtitle: data.statitem3_subtitle || DEFAULT_CONTENT.statItem3.subtitle
            }
          });
        }
      } catch (error) {
  ...
  // Determine whether to display dynamic title with HTML or as a string
  const renderTitle = () => {
    // Check if the title contains HTML (from Supabase or default)
    if (content.title.includes('<span')) {
      // If it's the default title with HTML styling, render it directly
      return <div dangerouslySetInnerHTML={{ __html: content.title.replace(/\${COLORS\.primary}/g, COLORS.primary).replace(/\${COLORS\.primaryLight}/g, COLORS.primaryLight) }} />;
    } else {
      // If it's a plain string from Supabase, apply the styling to "Project"
      const titleParts = content.title.split('Project');
      if (titleParts.length > 1) {
        return (
          <>
            {titleParts[0]}
            <span style={{ color: COLORS.primary, textShadow: `0 0 10px ${COLORS.primaryLight}` }}>
              Project
            </span>
            {titleParts[1]}
          </>
        ...
        // If "Project" is not in the title, return the title as is
        ...
        <div style={contentContainerStyles}>
          ...
          >
            ...
            <StatItem
              ...
              title={content.statItem1.title}
              subtitle={content.statItem1.subtitle}
              isVisible={isVisible}
              delay={0.6}
            ...
            <StatItem
              ...
              subtitle={content.statItem2.subtitle}
              highlighted={true}
              isVisible={isVisible}
              delay={0.8}
            ...
            <StatItem
              ...
              title={content.statItem3.title}
              subtitle={content.statItem3.subtitle}
              isVisible={isVisible}
              delay={1.0}
            isSmallMobile={isSmallMobile}
              isMobile={isMobile}
