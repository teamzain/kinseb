...
export default function BusinessServices() {
  ...
  const [autoScrollActive, setAutoScrollActive] = useState(true); // State to track if auto-scroll is active
  const autoScrollIntervalRef = useRef<NodeJS.Timeout | null>(null); // Reference for auto-scroll interval
  const autoScrollResumeTimerRef = useRef<NodeJS.Timeout | null>(null); // Reference for resume timer
  ...
  useEffect(() => {
    ...
    // Start auto-scrolling when component mounts and is visible
    if (isVisible) {
      startAutoScroll();
    }
  ...
  }, [isVisible, isMobile, autoScrollActive, cardSetIndex, maxScrollIndex, services.length, scrollPosition]);
