...
export default function BusinessServices() {
  ...
  const [dataFetched, setDataFetched] = useState(false); // Flag to track if data was fetched
  ...
  const createServicesArray = (data: BusinessSectionData): ServiceItem[] => {
    // Debug log to inspect the actual data coming in
    console.log("Creating services from data:", data);
    ...
    const fetchSectionData = async () => {
      try {
        ...
        if (!supabase) {
          ...
          setError('Database connection error: Client not initialized');
          setIsLoading(false);
          return;
        }
        ...
        if (supabaseError) {
          console.error('Error fetching business section data:', supabaseError);
          setError(`Database error: ${supabaseError.message}`);
          setDataFetched(false);
        } else if (data) {
          console.log('Successfully fetched business section data:', data);
          setSectionData(data);
          setDataFetched(true);
        } else {
          // Handle case where no data was returned but no error occurred
          console.warn('No business section data found');
  ...
  const services: ServiceItem[] = (dataFetched && sectionData) 
    ? createServicesArray(sectionData) 
    : defaultServices;
  
  // Log what data we're actually using
  useEffect(() => {
  ...
  }, [dataFetched, sectionData, services]);
