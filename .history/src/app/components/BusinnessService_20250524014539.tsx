...
const MobileServiceCard = ({ service, index, isActive, showHover, isVisible }: 
  ...
  // Completely disable hover on touch devices - only use timer-based hover
  const [isTouchDevice, setIsTouchDevice] = useState(false);
  const isHovered = isActive && showHover && !isTouchDevice;
  const router = useRouter();
  ...
  return (
    <div 
      ...
      style={{
        ...
        // Disable all touch interactions that could trigger hover
    ...
    >
      ...
      <div 
        ...
        style={{
          ...
          zIndex: 0,
          transform: isHovered ? 'scale(1.05)' : 'scale(1)',
          filter: isHovered ? 'brightness(1.1) contrast(1.2) saturate(1.1)' : 'brightness(0.8)', // Enhanced clarity
        }}>
        <Image
...
export default function BusinessServices() {
  ...
  // Effect to handle hover timer for mobile - DISABLED on touch devices
  useEffect(() => {
    if (!isMobile) return;
    
    // Detect if this is a touch device and disable timer hover completely
    ...
    // Start a new timer only for non-touch devices
      ...
      <style dangerouslySetInnerHTML={{
        __html: `
          ...
          @media (hover: none) and (pointer: coarse) {
            ...
            /* Disable all hover effects on mobile cards */
            .mobile-service-card:hover .card-overlay,
            .mobile-service-card:hover .card-background,
            .mobile-service-card:hover .card-content {
              opacity: inherit !important;
              transform: inherit !important;
            }
            
            /* Ensure mobile cards don't respond to hover at all */
            .mobile-service-card {
