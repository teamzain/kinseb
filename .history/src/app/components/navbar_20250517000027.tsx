...
const Navbar = () => {
  ...
  const servicesDialogRef = useRef<HTMLDivElement>(null);
  ...
  const toggleServicesDialog = (e: React.MouseEvent) => {
    e.preventDefault();
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        servicesDialogRef.current && 
        !servicesDialogRef.current.contains(event.target as Node) &&
  ...
  // Close services dialog when menu opens
  useEffect(() => {
    if (isMenuOpen) {
      setShowServicesDialog(false);
    }
  ...
  // Find the services link for the dialog
    ...
    <>
      ...
      <style jsx global>{`
       ...
       .menu-link-with-dropdown.active {
        ...
        /* Two column dropdown for ALL screen sizes - UPDATED */
        .dropdown-items-grid {
          display: grid;
        ...
        /* IMPROVED: Mobile dropdown scrollable */
        .mobile-dropdown-scroller {
          max-height: 300px;
          overflow-y: auto;
          margin-right: -10px;
          padding-right: 10px;
          scrollbar-width: thin;
          scrollbar-color: #0D98BA #091135;
        }

        .mobile-dropdown-scroller::-webkit-scrollbar {
          width: 6px;
        }

        .mobile-dropdown-scroller::-webkit-scrollbar-track {
          background-color: #091135;
          border-radius: 10px;
        }

        .mobile-dropdown-scroller::-webkit-scrollbar-thumb {
          background-color: #0D98BA;
          border-radius: 10px;
        }
        ...
        @media (max-width: 768px) {
          ...
          /* Hide desktop services dialog on mobile */
          .services-dialog {
            display: none;
          }
        }
      ...
      <nav className="navbar" role="navigation" aria-label="Main Navigation">
        <div className="container">
          ...
          <div 
            ...
            className={`services-dialog ${showServicesDialog ? 'open' : ''}`}
            aria-hidden={!showServicesDialog}
            role="dialog"
            aria-modal="true"
            aria-label="Services Menu"
          >
            <div className="services-dialog-container">
              <h3 className="services-dialog-title">Our Services</h3>
              <div className="services-grid">
                {servicesLink?.dropdownItems?.map((item, index) => (
                ...
                {link.hasDropdown ? (
                  <>
                    ...
                    <div className={`dropdown-menu ${openDropdown === link.id ? 'open' : ''}`}>
                      {/* UPDATED: Mobile scrollable dropdown */}
                      <div className="mobile-dropdown-scroller">
      ...
      {/* Desktop Services Dialog Trigger */}
      {!isMenuOpen && (
        <div 
          ...
          {navigationLinks.map((link) => {
            if (link.id === 'services') {
              return (
                <div 
                  key={link.id}
                  onClick={(e) => toggleServicesDialog(e)}
                  style={{ 
                    position: 'absolute',
                    top: '0',
                    left: '0',
