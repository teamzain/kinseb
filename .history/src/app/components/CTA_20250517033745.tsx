...
const StatsSection = () => {
  ...
  const animationRef = useRef(null); // Added to store animation frame reference
  ...
  // Fixed floating animation that preserves the initial transform
  useEffect(() => {
    // Only start floating animation after the card becomes visible
    if (isVisible && cardRef.current) {
      // Add subtle floating animation to card after entrance animation completes
      const startFloatingAnimation = () => {
        ...
        const animate = (timestamp) => {
          ...
          // Apply only the Y-axis floating animation without affecting other transforms
          // Use CSS variable to store the floating offset
          card.style.setProperty('--float-offset', `${translateY}px`);
      ...
      const floatTimer = setTimeout(startFloatingAnimation, 1500);
