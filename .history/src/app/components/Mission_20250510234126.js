...
const FigmaDesignComponent = () => {
  ...
  const getResponsiveStyles = () => {
    ...
    if (typeof window !== 'undefined' && viewportHeight > 0 && viewportWidth > 0) {
      ...
      // Calculate container height based on viewport dimensions
      // Fix for laptop: Ensure the aspect ratio preserves full image visibility
      const containerAspectRatio = viewportWidth >= 768 ? '2.2 / 1' : '1.8 / 1';
      ...
      // Calculate dynamic spacing between card and pagination
      const paginationSpace = viewportWidth < 768 
        ? Math.max(5, Math.min(10, viewportHeight * 0.01)) // Mobile: smaller space
        : Math.max(15, Math.min(30, viewportHeight * 0.03)); // Desktop: larger space
      ...
      return {
        frame: {
          ...
          opacity: isLoaded ? 1 : 0, // Start with opacity 0, transition to 1 when loaded
        ...
        carouselContainer: {
          ...
          // Added maxHeight to fix laptop cutoff issue
        ...
        carousel: {
          ...styles.carousel,
          height: 'auto',
          maxHeight: viewportWidth >= 768 ? 'calc(100vh - 240px)' : viewportHeight * 0.85, // Fixed laptop height
        },
        container: {
          ...styles.container,
          height: 'auto',
          minHeight: viewportWidth < 768 ? '420px' : '400px',
          maxHeight: viewportWidth >= 768 ? 'calc(100vh - 240px)' : viewportHeight * 0.85, // Fixed laptop height
          aspectRatio: containerAspectRatio, // Use the responsive aspect ratio
        },
        textContainer: {
          ...styles.textContainer,
          width: textContainerWidth,
          margin: viewportWidth < 768 
            ? '120px 0 30px 20px' // Increased from 80px to 120px - more space from top on mobile
        ...
        paginationContainer: {
          ...
          margin: `${paginationSpace}px auto`,
          marginTop: paginationSpace, // Consistent spacing
          marginBottom: paginationSpace, // Consistent spacing
          position: 'relative',
          zIndex: 10,
        },
        buttonsContainer: {
        ...
        // Add animations for card elements
        cardItem: {
          opacity: isVisible ? 1 : 0,
          transform: isVisible ? 'translateY(0)' : 'translateY(20px)',
          transition: 'opacity 0.8s ease-out 0.4s, transform 0.8s ease-out 0.4s'
        },
        // Background image style - modified to fix laptop display
        backgroundImageStyle: {
          backgroundSize: 'cover',
          backgroundPosition: 'center center', // Improved positioning
  ...
  return (
    <div style={{...responsiveStyles.frame}} className="figma-design-component">
      <div style={responsiveStyles.sectionFrame} ref={sectionRef} className="scroll-reveal-section">
        ...
        <div style={responsiveStyles.carouselContainer}>
          <div style={responsiveStyles.carousel} className="carousel">
            {cards.map((card, index) => (
              ...
              >
                {/* Separate background image div to ensure full coverage */}
                <div 
                  style={{
                    position: 'absolute',
                    top: 0,
      ...
      <style jsx global>{`
        /* Hide component initially to prevent flash */
...
const styles = {
  ...
  carousel: {
    position: 'relative',
    width: '100%',
    height: 'auto', // Changed to auto to adapt to container contents
    flex: '1 1 auto',
  },
  container: {
    ...
    minHeight: '400px', // Set minimum height for small screens
    aspectRatio: '1.8 / 1', // Maintain aspect ratio for consistent scaling
  ...
  textContainer: {
    position: 'relative',
    width: '535px',
    margin: '180px 0 0 75px', // Further decreased from 200px to 180px - less space from top for better fit
    zIndex: 2,
  },
  h2: {
    position: 'absolute',
