...
const TrustSection: React.FC<TrustSectionProps> = ({ 
 ...
 // Add this helper function at the top of your component, before the TrustSection component
const formatServiceTitle = (title: string): string => {
  // Remove " Services" suffix if it exists
  const cleanTitle = title.replace(' Services', '');
  
  // Split the title into words
  const words = cleanTitle.split(' ');
  
  // If 2 or fewer words, return as-is (single line)
  if (words.length <= 2) {
    return cleanTitle;
  }
  
  // If more than 2 words, put the first two words on first line,
  // and the rest on the second line
  ...
  return (
    <>
      ...
      <div style={{
        position: 'relative',
        width: '100%',
        minHeight: isMobile ? '1050px' : '800px',
...
>
  <motion.button
    onClick={() => handleServiceNavigation(tabs[activeTab].serviceId)}
    whileHover={{ 
      scale: 1.05,
      boxShadow: '0 8px 25px rgba(13, 152, 186, 0.3)'
    }}
  ...
  style={{
    flex: isMobile ? 'unset' : '1',
    width: '100%',
    // Remove the fixed height and replace with aspect ratio
    aspectRatio: '16/9', // or '4/3' for a more square format
    position: 'relative',
    borderRadius: '14px',
    overflow: 'hidden',
    ...
    cursor: 'pointer',
    // Add minimum dimensions to prevent images from becoming too small
...
>
  <Image 
    ...
    priority
    sizes="(max-width: 768px) 100vw, 50vw" // Optimize loading for different screen sizes
  />
