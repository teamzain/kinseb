  ...
  return (
    <div 
      ...
      style={{
        ...
        width: '100%',
        marginRight: '20px',
        marginLeft: '20px', // Add margin to both sides for mobile
      }}
    >
      {/* Background image */}
      <div style={{
...
const startAutoScroll = () => {
   ...
   const interval = setInterval(() => {
    if (viewportType === 'mobile') {
      setCurrentMobileIndex((prev) => {
        // If we're at the last card, move to the first after 4 seconds
        if (prev >= services.length - 1) {
          ...
          // Return current index to prevent immediate change
          return prev;
        }
      ...
      // Desktop and tablet carousel circular scrolling
      if (servicesRef.current) {
        const maxScrollSize = servicesRef.current.scrollWidth - servicesRef.current.clientWidth;
        
        // If we've reached the last set of cards
        if (cardSetIndex >= maxScrollIndex) {
          ...
          // Stay on last position for 4 seconds, then go back to first
          setTimeout(() => {
            setScrollPosition(0);
            servicesRef.current.scrollTo({
              left: 0,
  ...
  const scrollLeft = () => {
    ...
    if (servicesRef.current) {
      if (viewportType === 'mobile') {
        ...
        // Circular navigation - go to the end if
  ...
  if (servicesRef.current) {
    if (viewportType === 'mobile') {
      ...
      // Calculate the exact position needed to show the last cards
      if (cardSetIndex >= maxScrollIndex) {
        setScrollPosition(0);
        servicesRef.current.scrollTo({
          left: 0,
        ...
        // For the last card set, make sure we position exactly to show the last card
        const isSecondToLastPosition = cardSetIndex === maxScrollIndex - 1;
        let newPosition;
        
        if (isSecondToLastPosition) {
          // Ensure we position to show the last card
          newPosition = maxScrollSize;
        } else {
