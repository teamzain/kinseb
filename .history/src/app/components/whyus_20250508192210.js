...
const KinsebWebDevelopment = () => {
  ...
  // Handle document scroll to control card scrolling
  useEffect(() => {
    const handleWindowScroll = () => {
      ...
      if (sectionRect.top < viewportHeight && sectionRect.bottom > 0) {
        const sectionProgress = 1 - (sectionRect.top / (viewportHeight - sectionHeight));
        const cardProgress = Math.max(0, Math.min(1, sectionProgress));
        
        // Calculate relative position in the section to determine which card to show
        const cardIndex = Math.min(
          Math.floor(cardProgress * cards.length),
          cards.length - 1
        );
        
        // If card index changed, update the active card
        if (cardIndex !== activeCardIndex) {
          setActiveCardIndex(cardIndex);
          
          // Scroll to the corresponding card in the cards container
                  ...
                  <div style={{
                    ...
                    {index < cards.length - 1 && (
                      <button
                        ...
                        onClick={() => {
                          // Calculate new scroll position based on page scroll rather than card container
                          const nextCardIndex = index + 1;
                          setActiveCardIndex(nextCardIndex);
                          
                          // Calculate how much to scroll the window to show the next card
                          const sectionHeight = containerRef.current.offsetHeight;
                          const viewportHeight = window.innerHeight;
                          const scrollStep = (sectionHeight - viewportHeight) / (cards.length - 1);
                          
                          // Get the current section top position
                          const sectionTop = containerRef.current.getBoundingClientRect().top + window.pageYOffset;
                          
                          // Calculate target scroll position
                          const targetScroll = sectionTop + (scrollStep * nextCardIndex);
          ...
          {cards.map((card, index) => (
            <button
              ...
              onClick={() => {
                ...
                // Calculate how much to scroll the window to show the selected card
                const sectionHeight = containerRef.current.offsetHeight;
                const viewportHeight = window.innerHeight;
                const scrollStep = (sectionHeight - viewportHeight) / (cards.length - 1);
                ...
                // Calculate target scroll position
                const targetScroll = sectionTop + (scrollStep * index);
        ...
        <style jsx>{`
          ...
          * {
          ...
          @media (max-width: 1100px) {
            ...
            .service-card:nth-of-type(${activeCardIndex + 1}) {
