...
export default function BusinessServices() {
 ...
 const scrollLeft = () => {
  ...
  // Clean up timer when component unmounts or auto-scroll is deactivated
  return () => {
...
}, [cardSetIndex, isMobile, maxScrollIndex, autoScrollActive, services.length]);

 const scrollRight = () => {
  pauseAutoScroll();
  if (servicesRef.current) {
    // Your existing scrollRight code
    if (isMobile) {
      setCurrentMobileIndex((prev) => 
        prev >= services.length - 1 ? 0 : prev + 1
      );
