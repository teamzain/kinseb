  ...
  const getResponsiveStyles = () => {
    ...
    if (typeof window !== 'undefined' && viewportHeight > 0 && viewportWidth > 0) {
      ...
      return {
        ...
        carousel: {
          ...styles.carousel,
          height: viewportWidth < 768 ? 'auto' : `${Math.max(420, 720 * heightScaleFactor)}px`, // Increased height
        },
        container: {
          ...styles.container,
          height: viewportWidth < 768 ? 'auto' : `${Math.max(420, 720 * heightScaleFactor)}px`, // Increased height
          minHeight: viewportWidth < 768 ? '480px' : 'auto', // Decreased mobile height
        },
        textContainer: {
          ...styles.textContainer,
          width: textContainerWidth,
          margin: viewportWidth < 768 
            ? '60px 0 40px 20px' // Left aligned on mobile with proper spacing
            : `${Math.max(80, 160 * combinedFactor)}px 0 0 ${Math.max(30, 75 * combinedFactor)}px`,
          padding: viewportWidth < 768 ? '0 15px' : '0',
        },
        h2: {
          ...styles.h2,
          fontSize: viewportWidth < 768 ? '100px' : `${Math.max(80, 150 * combinedFactor)}px`, // Larger on mobile for consistency
          lineHeight: viewportWidth < 768 ? '42px' : `${Math.max(40, 64 * combinedFactor)}px`,
          top: viewportWidth < 768 ? '-45px' : `${-64 * combinedFactor}px`,
          width: viewportWidth < 768 ? '100%' : '782px',
          left: viewportWidth < 768 ? '-5px' : '-11px', // Better positioning on mobile
        },
        heading: {
          ...styles.heading,
          fontSize: viewportWidth < 768 ? '32px' : `${Math.max(25, 35 * combinedFactor)}px`, // Larger on mobile for consistency with desktop
          lineHeight: viewportWidth < 768 ? '40px' : `${Math.max(30, 45 * combinedFactor)}px`,
          marginBottom: viewportWidth < 768 ? '20px' : `${Math.max(15, 30 * combinedFactor)}px`,
          marginTop: viewportWidth < 768 ? '50px' : `${Math.max(50, 84 * combinedFactor)}px`, // More space at top
          width: viewportWidth < 768 ? '95%' : '572px', // Slightly less than 100% on mobile
        },
        paragraph: {
          ...styles.paragraph,
          fontSize: viewportWidth < 768 ? '18px' : `${Math.max(14, 18 * combinedFactor)}px`, // Same size as desktop
          lineHeight: viewportWidth < 768 ? '150%' : '150%', // Consistent line height
          marginBottom: viewportWidth < 768 ? '30px' : `${Math.max(20, 40 * combinedFactor)}px`,
          width: viewportWidth < 768 ? '90%' : '533px', // Slightly less than 100% on mobile
        },
        paginationContainer: {
        ...
        button: {
          ...styles.button,
          width: viewportWidth < 768 ? '160px' : '157px', // Slightly wider on mobile
        }
      };
    }
            ...
            >
              ...
              <div style={responsiveStyles.textContainer}>
                ...
                <div style={responsiveStyles.buttonsContainer}>
                  <button style={responsiveStyles.button || styles.button}>
...
const styles = {
  ...
  carousel: {
    position: 'relative',
    width: '100%',
    height: '920px', // Increased height for laptop view
    flex: '1 1 auto',
  },
  container: {
