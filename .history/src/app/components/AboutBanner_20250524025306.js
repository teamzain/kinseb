...
const Banner = () => {
  ...
  const headerStyles = useMemo(() => ({
    ...
    alignItems: 'center',
    width: '100%',
    marginTop: isSmallMobile ? '40px' : isMobile ? '60px' : isTablet ? '50px' : '30px', // Increased margin-top for header
  }), [isSmallMobile, isMobile, isTablet]);
  ...
  const subtitleStyles = useMemo(() => ({
    ...
    marginTop: isSmallMobile ? '30px' : isMobile ? '35px' : isTablet ? '40px' : '45px', // Added margin-top for subtitle
    textAlign: 'center',
    color: COLORS.primary,
