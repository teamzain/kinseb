...
const KinsebWebDevelopment = () => {
  ...
  // and prevent scrolling beyond the last card
  useEffect(() => {
    const handleScroll = () => {
      ...
      const isAtEnd = rawProgress >= cards.length - 0.5;
      
      // If we're at the end, prevent further scrolling by using a scroll boundary
      if (isAtEnd && !hasReachedEnd) {
        setHasReachedEnd(true);
        
        // Calculate the exact position where scrolling should stop
        const endScrollPosition = scrollStart + (totalScrollHeight * ((cards.length - 0.5) / cards.length));
        
        // Smoothly scroll to that position
        window.scrollTo({
          top: endScrollPosition,
          behavior: 'smooth'
        });
        
        // Reset after a short delay to allow the smooth scroll to complete
        setTimeout(() => {
  ...
  }, [isVisible, cards.length, activeCardIndex, hasReachedEnd]);

  // Add wheel event listener to prevent scrolling past the last card
  useEffect(() => {
    const handleWheel = (e) => {
      ...
      // Calculate the position where we want to stop scrolling (just after the last card)
      const stopPosition = containerTop + (containerRef.current.offsetHeight * 0.85);
      
      // Check if we're at the end of the component and trying to scroll down
      if (currentScroll >= stopPosition && e.deltaY > 0 && activeCardIndex === cards.length - 1) {
        // Prevent the default scroll behavior
        e.preventDefault();
        
        // Keep the user at the end of our component
        window.scrollTo({
          top: stopPosition,
          behavior: 'auto'
        });
  ...
  return (
    <>
      ...
      >
        {/* Scroll Boundary Element - Invisible div to help manage scroll behavior */}
        <div 
          id="scroll-boundary" 
          style={{
            position: 'absolute',
        ...
        <div className="services-container" style={{
          ...
          <div 
            ...
            style={{
              flex: '1 1 500px',
              position: 'relative',
              minHeight: '200vh', // Reduced height to prevent excessive scrolling
              maxHeight: '250vh', // Add max height to help control scroll area
              display: 'flex',
              flexDirection: 'column',
              paddingTop: '15vh',
              paddingBottom: '20vh',
          ...
          >
            ...
            {/* Reduced number of spacer elements to prevent excessive scrolling */}
