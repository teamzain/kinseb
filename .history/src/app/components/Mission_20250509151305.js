...
const lato = Lato({
  ...
  useEffect(() => {
    ...
    const updateScale = () => {
      ...
      // For mobile (width < 768px), use a higher minimum scale to ensure cards are visible
      const minScale = viewportWidth < 768 ? 0.5 : 0.4;
  ...
  const scaledStyles = {
    ...
    carouselContainer: applyScale({
      ...
      width: isMobile ? '100%' : isLaptop ? '100%' : styles.carouselContainer.width
    }),
    carousel: applyScale({
      ...styles.carousel,
      // Ensure the carousel takes full height on mobile
      height: isMobile ? '520px' : styles.carousel.height
    }),
    container: applyScale({
      ...styles.container,
      // Ensure the container takes full height on mobile
      height: isMobile ? '520px' : styles.container.height
    }),
    overlay: styles.overlay,
    mobileOverlay: styles.mobileOverlay,
    textContainer: applyScale({
      ...styles.textContainer,
      // Adjust text container width and positioning based on screen size
      width: isMobile ? '90%' : isLaptop ? '60%' : styles.textContainer.width,
      margin: isMobile ? '90px 0 0 20px' : isLaptop ? '140px 0 0 60px' : styles.textContainer.margin
    }),
    h2: applyScale({
      ...styles.h2,
      // Make the title text more visible on different screens
      color: isMobile ? 'rgba(13, 152, 186, 0.5)' : styles.h2.color
    }),
    heading: applyScale({
      ...styles.heading,
      // Adjust heading width on different screens
      width: isMobile ? '100%' : styles.heading.width
    }),
    solutions: styles.solutions,
    paragraph: applyScale({
      ...styles.paragraph,
      // Adjust paragraph width based on screen size
  ...
  // Adjust card visibility based on screen size
  const getCardStyles = (index) => {
    const isActive = index === activeCard;
    
    // Get content for the current card
    const cardContent = cards[index];
    const cardTitle = cardContent.title.toLowerCase();
    ...
    return {
      ...
      filter: isMobile ? 'brightness(1.3) contrast(1.1)' : 'brightness(1.1)',
      // Ensure the container takes full width/height
      width: '100%',
      height: '100%'
    };
  };
        ...
        <div style={scaledStyles.carousel} className="carousel">
          {cards.map((card, index) => (
            ...
            >
              ...
              <div style={screenSize.width < 768 ? scaledStyles.mobileOverlay : scaledStyles.overlay}></div>
...
const styles = {
  ...
  carouselContainer: {
    ...
    maxWidth: '1400px', // Increase from 1202px
    position: 'relative',
    flex: '1 1 auto',
    display: 'flex',
    flexDirection: 'column',
