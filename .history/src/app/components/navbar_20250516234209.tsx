...
const Navbar = () => {
  ...
  const [servicesDialogPosition, setServicesDialogPosition] = useState({ top: 0, left: 0 });
  ...
  const toggleDropdown = (id: string, e?: React.MouseEvent) => {
    ...
    // For desktop behavior, keep the original dropdown toggle
    ...
    // For mobile, implement the dialog behavior for Services
    if (id === 'services') {
      if (showServicesDialog) {
        setShowServicesDialog(false);
      } else {
        // Get the position of the clicked element to position the dialog
        const button = document.getElementById(`dropdown-toggle-${id}`);
        if (button) {
          const rect = button.getBoundingClientRect();
          const top = rect.top + rect.height / 2; // Center vertically with the button
          const left = 0; // Left side of the screen
          
          setServicesDialogPosition({ top, left });
    ...
    const handleClickOutside = (e: MouseEvent) => {
      if (showServicesDialog) {
        ...
        if (dialog && !dialog.contains(e.target as Node)) {
          const servicesDropdownToggle = document.getElementById('dropdown-toggle-services');
          // Don't close if clicking the toggle button itself
          if (!servicesDropdownToggle || !servicesDropdownToggle.contains(e.target as Node)) {
            setShowServicesDialog(false);
          }
        }
      }
  ...
  // Find services link data for dialog
