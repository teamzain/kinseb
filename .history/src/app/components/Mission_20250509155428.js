  ...
  const getResponsiveStyles = () => {
    ...
    if (typeof window !== 'undefined' && viewportHeight > 0 && viewportWidth > 0) {
      ...
      return {
        ...
        mainHeading: {
          ...styles.mainHeading,
          fontSize: viewportWidth < 768 ? '30px' : `${Math.max(40, 65 * combinedFactor)}px`, // Smaller on mobile
          lineHeight: viewportWidth < 768 ? '38px' : `${Math.max(60, 98 * combinedFactor)}px`,
        ...
        textContainer: {
          ...
          margin: viewportWidth < 768 
            ? '100px 0 40px 20px' // More space from top on mobile
            : `${Math.max(200, 240 * combinedFactor)}px 0 0 ${Math.max(30, 75 * combinedFactor)}px`, // More space from top on laptop
          padding: viewportWidth < 768 ? '0 15px' : '0',
        },
        h2: {
          ...styles.h2,
          fontSize: viewportWidth < 768 ? '80px' : `${Math.max(80, 150 * combinedFactor)}px`, // Smaller on mobile
        ...
        heading: {
          ...styles.heading,
          fontSize: viewportWidth < 768 ? '24px' : `${Math.max(25, 35 * combinedFactor)}px`, // Smaller on mobile
          lineHeight: viewportWidth < 768 ? '32px' : `${Math.max(30, 45 * combinedFactor)}px`,
          marginBottom: viewportWidth < 768 ? '16px' : `${Math.max(15, 30 * combinedFactor)}px`,
          marginTop: viewportWidth < 768 ? '40px' : `${Math.max(50, 84 * combinedFactor)}px`,
          width: viewportWidth < 768 ? '95%' : '572px',
        },
        paragraph: {
          ...styles.paragraph,
          fontSize: viewportWidth < 768 ? '15px' : `${Math.max(14, 18 * combinedFactor)}px`, // Smaller on mobile
          lineHeight: viewportWidth < 768 ? '150%' : '150%',
          marginBottom: viewportWidth < 768 ? '25px' : `${Math.max(20, 40 * combinedFactor)}px`,
          width: viewportWidth < 768 ? '90%' : '533px',
        },
        paginationContainer: {
...
const styles = {
  ...
  textContainer: {
    position: 'relative',
    width: '535px',
    margin: '240px 0 0 75px', // Changed from 160px to 240px - more space from top
    zIndex: 2,
  },
  h2: {
    position: 'absolute',
