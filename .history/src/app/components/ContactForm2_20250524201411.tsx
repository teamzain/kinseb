...
export default function ContactForm2({ serviceId }: ContactForm2Props) {
  ...
  const currentService = useMemo(() => {
    ...
    let service = servicesData.services.find((service: Service) => service.id === serviceId);
    
    // If not found and serviceId is numeric, try converting to number
    if (!service && !isNaN(Number(serviceId))) {
      const numericId = Number(serviceId);
      service = servicesData.services.find((service: Service) => Number(service.id) === numericId);
    }
      ...
      <main className="landing-container" role="main">
        ...
        >
          <div className="content-wrapper">
            ...
            {/* FAQ sections with proper semantics - Updated to display titles from JSON */}
