...
// Fix 1: Update the TrustSectionProps interface to accept both string and number
interface TrustSectionProps {
  excludeServiceId?: string | number; // Accept both types for flexibility
}
...
useEffect(() => {
  if (trustData?.trustSection?.services) {
    ...
    if (excludeServiceId !== undefined) {
      // Convert excludeServiceId to string for comparison since service.id is string
      const excludeIdString = String(excludeServiceId);
      
      // Check if the excludeServiceId exists in the services
      const serviceExists = trustData.trustSection.services.some(service => 
        service.id === excludeIdString || service.id === excludeServiceId
      );
      
      if (serviceExists) {
        // Exclude the specific service and show all others
        filtered = trustData.trustSection.services.filter(service => 
          service.id !== excludeIdString && service.id !== excludeServiceId
        );
      } else {
...
const handleSeeService = (serviceId: string) => { // Changed from number to string
