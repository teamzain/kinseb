...
const FigmaDesignComponent = () => {
  ...
  useEffect(() => {
    // Update viewport height and check device type on mount and resize
    const updateViewportDimensions = () => {
      setViewportHeight(window.innerHeight);
      // Consider laptop screens to be between 768px and 1440px
  ...
  const getResponsiveStyles = () => {
    ...
    if (typeof window !== 'undefined' && viewportHeight > 0) {
      const scaleFactor = Math.min(1, viewportHeight / 960);
      
      // Additional vertical offset for laptop screens
      const laptopTopOffset = isLaptop ? 40 : 0; // Additional pixels to move content down on laptop screens
      ...
      return {
        ...
        carousel: {
          ...styles.carousel,
          height: `${Math.max(320, 620 * scaleFactor)}px`,
        },
        container: {
          ...styles.container,
          height: `${Math.max(300, 620 * scaleFactor)}px`,
        },
        textContainer: {
          ...styles.textContainer,
          margin: `${Math.max(80, 160 * scaleFactor) + laptopTopOffset}px 0 0 ${Math.max(30, 75 * scaleFactor)}px`,
        },
        h2: {
          ...styles.h2,
