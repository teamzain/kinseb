  ...
  // Dynamically calculate sizes based on viewport dimensions
    ...
    if (typeof window !== 'undefined' && viewportHeight > 0 && viewportWidth > 0) {
      ...
      if (viewportWidth >= 1024) {
        textContainerWidth = '50%'; // Decreased from 60%
      } else if (viewportWidth >= 768) {
        textContainerWidth = '65%'; // Decreased from 75%
      } else {
        textContainerWidth = '85%'; // Decreased from 90%
      }
      ...
      return {
        ...
        carousel: {
          ...styles.carousel,
          height: viewportWidth < 768 ? 'auto' : `${Math.max(350, 700 * heightScaleFactor)}px`, // Increased height
        },
        container: {
          ...styles.container,
          height: viewportWidth < 768 ? 'auto' : `${Math.max(350, 700 * heightScaleFactor)}px`, // Increased height
          minHeight: viewportWidth < 768 ? '400px' : 'auto', // Decreased from 500px for mobile
        },
        textContainer: {
          ...styles.textContainer,
          width: textContainerWidth,
          // For mobile, center the text container horizontally
          margin: viewportWidth < 768 
            ? '60px auto 30px auto' // Centered with auto margins and reduced top/bottom margins
            : `${Math.max(80, 160 * combinedFactor)}px 0 0 ${Math.max(30, 75 * combinedFactor)}px`,
          padding: viewportWidth < 768 ? '0 15px' : '0',
          textAlign: viewportWidth < 768 ? 'center' : 'left', // Center text on mobile
        },
        h2: {
          ...styles.h2,
          fontSize: viewportWidth < 768 ? '70px' : `${Math.max(80, 150 * combinedFactor)}px`,
          lineHeight: viewportWidth < 768 ? '34px' : `${Math.max(40, 64 * combinedFactor)}px`,
          top: viewportWidth < 768 ? '-35px' : `${-64 * combinedFactor}px`,
          width: viewportWidth < 768 ? '100%' : '782px',
          left: viewportWidth < 768 ? '0' : '-11px',
          textAlign: viewportWidth < 768 ? 'center' : 'left', // Center on mobile
          position: viewportWidth < 768 ? 'relative' : 'absolute', // Change to relative on mobile
        },
        heading: {
          ...styles.heading,
          fontSize: viewportWidth < 768 ? '24px' : `${Math.max(25, 35 * combinedFactor)}px`,
          lineHeight: viewportWidth < 768 ? '30px' : `${Math.max(30, 45 * combinedFactor)}px`,
          marginBottom: viewportWidth < 768 ? '15px' : `${Math.max(15, 30 * combinedFactor)}px`,
          marginTop: viewportWidth < 768 ? '10px' : `${Math.max(50, 84 * combinedFactor)}px`, // Reduced top margin on mobile
          width: '100%',
          textAlign: viewportWidth < 768 ? 'center' : 'left', // Center on mobile
        },
        paragraph: {
        ...
        buttonsContainer: {
          ...
          marginBottom: viewportWidth < 768 ? '30px' : '20px',
          justifyContent: viewportWidth < 768 ? 'center' : 'flex-start', // Center button on mobile
        }
      };
    }
        ...
        <div style={responsiveStyles.carousel} className="carousel">
          {cards.map((card, index) => (
            ...
            >
              ...
              <div style={{
                ...styles.overlay,
                background: viewportWidth < 768 
                  ? 'linear-gradient(0deg, rgba(4, 9, 29, 0.85) 0%, rgba(4, 9, 29, 0.7) 100%)' 
...
const styles = {
  ...
  carouselContainer: {
    width: '85%', // Decreased from 95%
    maxWidth: '1100px', // Decreased from 1400px
    position: 'relative',
    flex: '1 1 auto',
    display: 'flex',
    flexDirection: 'column',
  ...
  carousel: {
    position: 'relative',
    width: '100%',
    height: '950px', // Increased from 880px
    flex: '1 1 auto',
  },
  container: {
