...
export default function BusinessServices() {
  ...
  const [currentCardIndex, setCurrentCardIndex] = useState(0); // Track the current card index for single card scrolling
  ...
  const [autoScrollActive, setAutoScrollActive] = useState(true); // Control if auto-scroll is active
  const [autoScrollInterval, setAutoScrollInterval] = useState<NodeJS.Timeout | null>(null);
  const [loading, setLoading] = useState(true);
  const [sectionData, setSectionData] = useState<BusinessSectionData | null>(null);
  const [userInteracted, setUserInteracted] = useState(false); // Track if user has interacted with carousel
  ...
  // Calculate card width for single card scrolling
  const cardWidth = isMobile ? totalWidth : totalWidth / visibleCards;
  ...
  // Scroll to current card index for desktop
  useEffect(() => {
    if (!isMobile && servicesRef.current && cardWidth > 0) {
      const scrollTo = currentCardIndex * cardWidth;
      servicesRef.current.scrollTo({
        left: scrollTo,
        behavior: 'smooth'
      });
  ...
  }, [currentCardIndex, cardWidth, isMobile]);
  
  // Set up auto-scroll functionality that scrolls one card at a time
  useEffect(() => {
    // Only start auto-scrolling when the section is visible and it's active
    if (!isVisible || !autoScrollActive || userInteracted) return;
    
    // Start auto-scrolling
    const interval = setInterval(() => {
      if (isMobile) {
        // Mobile scrolling - one card at a time
        setCurrentMobileIndex((prev) => 
          prev >= services.length - 1 ? 0 : prev + 1
        );
      } else {
        // Desktop scrolling - one card at a time
  ...
  // Handle user interaction - stop auto-scrolling when user interacts
  ...
  // Scroll controls
  const scrollLeft = () => {
    handleUserInteraction(); // Stop auto-scrolling on user interaction
