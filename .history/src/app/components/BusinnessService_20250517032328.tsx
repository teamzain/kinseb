...
export default function BusinessServices() {
  // State for section data from Supabase
  const [sectionTitle, setSectionTitle] = useState<string>("Your Business Is Unique");
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Query the homebuisnesssection table
        ...
        if (error) {
          ...
          // Update state with fetched data if it exists
  ...
  // Extract business name and highlight color from section title
  // This logic will split the title to keep the same styling as original when dynamic title is loaded
  let businessText = "Business";
  let remainingTitle = "Is Unique";
  
  // Process title for highlighting the word "Business" (or other word if customized in database)
  // We'll keep the styling consistent with the original implementation
 const processTitle = (title: string): React.ReactNode => {
  // If there's no title or error fetching, use default fallback
  if (!title) return null;
  
  // Try to find the word "Business" (case insensitive) in the title
  const businessRegex = /\b(business)\b/i;
  const match = title.match(businessRegex);
  
  if (match) {
    // Split around the matched word to get before and after portions
    const matchedWord = match[0]; // The matched word (preserving the original case)
    const matchIndex = title.indexOf(matchedWord);
    const beforeText = title.substring(0, matchIndex);
    remainingTitle = title.substring(matchIndex + matchedWord.length);
    
    // Return JSX with the word "Business" highlighted
    return (
      <>
        {beforeText}<span style={{ color: '#36b2c8' }}>{matchedWord}</span>{remainingTitle}
      </>
    );
  } else {
    // If "Business" is not in the title, find the middle word to highlight instead
    const words = title.split(' ');
    const middleIndex = Math.floor(words.length / 2);
    
    // Rebuild the title with the middle word highlighted
    return (
      <>
        {words.slice(0, middleIndex).join(' ')} <span style={{ color: '#36b2c8' }}>{words[middleIndex]}</span> {words.slice(middleIndex + 1).join(' ')}
      </>
    );
  }
};
        ...
        <div style={carouselContainerStyle}>
          ...
          >
            {isMobile ? (
              ...
              // Desktop view - multiple cards with animation
              services.map((service, index) => (
                <ServiceCard 
                  key={index}
          ...
          {/* Mobile-only navigation dots and buttons */}
          {isMobile && (
            <>
              <div style={navigationContainerStyle}>
                <button 
                ...
                {services.map((_, index) => (
                  <div
                    ...
                    style={{
                      ...
                      height: '10px',
                      borderRadius: '50%',
                      backgroundColor: currentMobileIndex === index ? '#0D98BA' : '#ffffff3d',
                      margin: '0 5px',
                      cursor: 'pointer',
