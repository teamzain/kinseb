...
const StatItem = ({ icon, title, subtitle, highlighted = false, delay, isVisible, counter, isSmallMobile, isMobile, isTablet }) => {
  const statItemStyle = useMemo(() => ({
    ...
    boxShadow: highlighted ? '0 6px 24px rgba(0, 0, 0, 0.1), 0 0 0 1px rgba(255, 255, 255, 0.05) inset' : 'none',
  ...
  return (
    ...
    >
      <div className="stat-icon"
        style={{
          width: isSmallMobile ? '50px' : isMobile ? '60px' : isTablet ? '65px' : '70px', // Smaller icons
          height: isSmallMobile ? '50px' : isMobile ? '60px' : isTablet ? '65px' : '70px', // Smaller icons
...
const Banner = () => {
  ...
  const bannerStyles = useMemo(() => ({
    ...
    minHeight: isSmallMobile ? '700px' : isMobile ? '680px' : '580px', // Reduced min-height
  ...
  const headingStyles = useMemo(() => ({
    ...
    textAlign: 'center',
    color: COLORS.white,
    margin: isSmallMobile ? '15px 0 0' : isMobile ? '0px 0 0' : isTablet ? '20px 0 0' : '1px 0 0',
