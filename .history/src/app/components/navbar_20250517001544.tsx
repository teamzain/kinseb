...
const Navbar = () => {
  ...
  const toggleServicesDialog = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    setIsServicesDialogOpen(!isServicesDialogOpen);
  };
...
.navbar .menu-side .menu-header .menu-close svg {
  ...
  left: auto !important;
        ...
        /* Desktop Services Link - new styling */
        .desktop-services-link {
          position: relative;
          cursor: pointer;
        }

        .desktop-services-link:hover .menu-link {
          color: #00BCD4;
        }

        /* Services Dialog Box - New Addition */
        .services-dialog {
          position: absolute;
          top: 100%;
          left: 0;
...
.dropdown-toggle.active svg path {
  stroke: #00BCD4;
       ...
       .menu-link-with-dropdown.active {
        ...
        /* Hide services dialog on mobile */
      ...
      <nav className="navbar" role="navigation" aria-label="Main Navigation">
        ...
        <div className="desktop-navigation" style={{ display: 'flex', justifyContent: 'center' }}>
          <div style={{ 
            display: 'flex', 
            maxWidth: '1400px', 
            width: '100%', 
            ...
            <ul style={{ 
              ...
              {navigationLinks.map((link) => (
                <li 
                  key={link.id} 
                  className={link.hasDropdown ? "desktop-services-link" : ""}
                  ref={link.id === 'services' ? servicesButtonRef : null}
                >
                  {link.id === 'services' ? (
                    <div
                      className="menu-link"
                      ...
                      onKeyDown={(e) => {
                        if (e.key === 'Enter' || e.key === ' ') {
                          toggleServicesDialog(e as React.MouseEvent<HTMLDivElement>);
                        }
                      }}
                    >
                      ...
                      {link.label}
                      <span className={`dropdown-arrow ${isServicesDialogOpen ? 'open' : ''}`} style={{ marginLeft: '5px' }}>
                  ...
                  {/* Services Dialog Box - Only for desktop */}
                  {link.id === 'services' && (
                    <div 
                      ref={dialogRef}
                      className={`services-dialog ${isServicesDialogOpen ? 'open' : ''}`}
                      style={{ 
                        left: '-200px', // Position to left side
                        top: 'calc(100% + 0px)' // No gap between navbar and dialog
                      }}
                    >
