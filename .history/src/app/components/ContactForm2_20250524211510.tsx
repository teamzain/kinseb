...
export default function ContactForm2({ serviceId }: ContactForm2Props) {
  ...
  const faqStructuredData = useMemo(() => ({
    "@context": "https://schema.org",
    "@type": "FAQPage",
   "mainEntity": (currentService.faq || []).map((faq: { question: string; answer: string }) => ({
      "@type": "Question",
      "name": faq.question,
      "acceptedAnswer": {
        "@type": "Answer",
    ...
    <>
      ...
      <main className="landing-container" role="main">
        ...
        >
         ...
         {currentService.faq.map((faqItem: { question: string; answer: string }, index: number) => (
                  <article 
                    key={`faq-${index}`}
                  ...
                  >
                    <button 
                      ...
                      aria-expanded={expandedSection === `faq-${index}`}
                    ...
                    >
                      ...
                      <div 
                        className={`faq-arrow ${expandedSection === `faq-${index}` ? 'rotated' : ''}`}
                        aria-hidden="true"
                      >
